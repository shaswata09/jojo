 ...
 *
 * The phone's half of web's `lib/cv-agent.ts`, and the same file twice for the
 * reason its header gives: the two things that differ between the platforms —
 * opening a stored document and sending a turn — are both already behind
 * imports that resolve to this app's own copies.
 *
 * The reasoning is on the web file and is not repeated: why it is several calls
 * rather than one, why a failed pass costs that pass and not the whole CV, and
 * why nothing here writes.
 *
 * One thing IS worth restating on the phone: a full academic CV is now up to
 * nine round trips rather than one. On a laptop talking to a local model that
 * is seconds; on a phone talking to a cloud provider over mobile data it is
 * somebody's battery and somebody's tokens. `MAX_PASSES` is the bound, the step
 * labels say which pass is running, and the caller's `signal` stops it — all
 * three matter more here than they do in a browser.
 */
