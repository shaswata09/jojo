...
export type FileProps = {
  ...
  note?: string
  /**
   * Where the document actually is on this device. NATIVE ONLY — see below.
   *
   * ══ The one divergence from the web app's copy of this file ══
   *
   * D27 says no bytes are held, and this keeps that: the graph holds a path,
   * never content. What changes is that on a phone there is somewhere for a
   * path to point. The picker copies the chosen file into the app's own
   * document directory, and this records where it went, so a record can be
   * opened rather than merely described.
   *
   * The web app has no equivalent and does not need one — it holds a `File`
   * for the session and hands it to an `<iframe>`, which is a browser
   * affordance with no counterpart here. So this field is additive, optional,
   * and invisible to every existing reader: a record without it behaves
   * exactly as every record did before.
   *
   * If `src/kg` is ever re-copied from the web app, this is the edit to
   * reapply. It is the only one.
   */
  uri?: string
}
