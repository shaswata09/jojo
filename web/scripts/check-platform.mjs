 ...
 * on all three. Nothing enforced that. `tsconfig.kg.json` extends the app
 * config, which sets `"lib": ["ES2023", "DOM"]`, so `window.addEventListener`
 * compiled clean; `check-layers.mjs` matches module specifiers, and a global is
 ...
 * in `src/lib`, which is web-only and allowed to be.
 *
 * Since then the type system carries the same rule: `tsconfig.kg.json` and
 * `tsconfig.kg-react.json` compile those layers with `"lib": ["ES2023"]` and no
 ...
 * setTimeout anywhere under src/kg" and "no setTimeout in core and tools, where
 ...
 * this codebase has all four: of 28 naive-grep hits under src/kg, 23 are prose
 ...
 * cannot express it. `tsconfig.kg-react.json` sets `"types": ["react"]`, and
 ...
 *
 * Per layer, not one flat rule over src/kg, because the layers genuinely differ.
 ...
 * domain-noun hits under src/kg lands in this function or in `boundNames`.
 *
 ...
 * correct site under src/kg is `new Date(injectedInstant)`. The difference
...
console.log('check-platform: src/kg and src/data are free of platform APIs and wall-clock reads')
