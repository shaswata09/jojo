 ...
 *
 * `tsconfig.kg.json` and `tsconfig.kg-react.json` compile `src/kg/{core,repo,
 ...
 *
 * This file is NOT under `src`, and that is load-bearing. `tsconfig.app.json`
 * includes `src` wholesale and keeps DOM, so a copy of these declarations inside
 * `src/kg` would collide with `lib.dom.d.ts` on every one of them — "Subsequent
 * variable declarations must have the same type" — and the fix someone reaches
 * for under that error is to widen the shapes back to DOM's.
 *
