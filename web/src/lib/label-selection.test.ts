...
import labelsSource from './labels.tsx?raw'
...
describe('the provider that applies it', () => {
  /** Comments stripped: the prose below quotes both spellings while explaining them. */
  const source = labelsSource.replace(/\/\*[\s\S]*?\*\//g, '').replace(/^\s*\/\/.*$/gm, '')

  it('derives the lit set from the keywords that exist', () => {
    expect(source).toContain('litSelection(selected, labels)')
  })

  it('filters on the derived set and publishes it as the selection', () => {
    // Both halves. `matches` is what empties the lists; `selected: lit` is what
    // the chips and the counts read, and either one left on the raw set
    // reinstates the defect on its own surface.
    expect(source).toMatch(/lit\.size === 0 \|\| carries\(recordId, lit\)/)
    expect(source).toContain('selected: lit')
  })

  it('is reading a source that is really there', () => {
    // Guards the guard: every assertion above is a substring test, and a `?raw`
    // import that resolved to an empty string would fail all three loudly —
    // but one that resolved to the WRONG file would not, so this names two
    // things only this provider has.
    expect(source).toContain('LabelsContext.Provider')
    expect(source).toContain('useKeywords()')
  })
})
