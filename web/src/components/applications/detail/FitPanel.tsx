...
export function FitPanel({ applicationId }: { applicationId: string }) {
  ...
  /*
   * The automatic read. Guarded on `requirements` being null so it runs once
   * per document per session — and on `error`, because a posting that failed to
   * read fails the same way on every render, and retrying it without being
   * asked would be a loop against somebody's GPU.
   */
  useEffect(() => {
  ...
  // the last one is wrong rather than stale. Without this, opening a second
  // record shows the first one's requirements until the effect resolves.
  useEffect(() => {
