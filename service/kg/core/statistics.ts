...
/*
 * These four came in through `@/data/seed` and `@/data/timeline` for as long as
 * this file sat in `src/data` — but the fixtures never owned any of them. Every
 * one was already a re-export of the module beside this one, so the edge went
 * core -> data -> core and only looked like a dependency on the demo dataset.
 * Spelling it at the source is what let this file move down into `core`, where
 * the layer guard bans importing anything else at all.
 */
import type { Application, RoleTag, TimelineItem } from './model'
...
/**
 * The count at a funnel step, 0 for a step that is not there.
 *
 * `funnel[0].count` was written directly at four sites, and it was correct —
 * `funnelFor` maps over the five `FUNNEL_STEPS`, so all five always exist. It
 * only became a compile error when this file moved out of the web app and under
 * `noUncheckedIndexedAccess` with the rest of `kg`. Spelling the fallback once
 * beats four `?? 0`s, and 0 is the right answer anyway: every one of these is a
 * numerator or a denominator, and a step that does not exist has not been
 * reached by anybody.
 */
...
function kpisFor(all: readonly Application[], funnel: FunnelStep[]): Kpi[] {
  ...
  const rate = (label: string, step: number, verb: string, typical: number): Kpi => ({
    label,
    value: `${pct(countAt(funnel, step), sent)}%`,
    note: `${countAt(funnel, step)} of ${sent} ${verb}`,
