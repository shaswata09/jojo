...
```
jojo/
├── web/                 React single-page app — the only implemented piece today
├── mobile/              (empty) planned mobile client
...
background scroll locks, and focus returns to the hamburger. Above `lg` it must *not* claim those
...
the mobile client, and any test suite.
