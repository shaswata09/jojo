...
- **~~`mobile/`** — the Expo app is not wired to the store.~~ REVISED — Expo is gone (see `docs/EXPO-REMOVAL.md`) and the phone app is wired: `mobile/src/lib/store.tsx` calls `boot({ now, driver: createRnDriver() })`, and there are fifteen screens over it. The portability work (D26, the `Host`/`Driver`/toast ports, `check-platform.mjs`) is what makes it a wiring job when it happens.
...
- **~~No cross-device transfer.~~ REVISED — this shipped.** The scope line held for one release and then stopped being true. Transfer is `core/handoff.ts`, `core/pairing.ts`, `core/convoy.ts` and `core/pulse.ts`, with an X25519 + HKDF layer of its own under `kg/crypto/`, a route at `web/src/routes/Transfer.tsx` and a LAN listener at `mobile/src/lib/handoff-server.ts`.

  What the original line got RIGHT is the part worth keeping: there is still no sync protocol, no merge, no CRDT and no vector clocks. Transfer is one-directional, manual and on demand — a whole store is sealed, carried and replaces the one at the other end. `core/handover.ts` exists precisely because that is not sync and the app owes the user the drift number before it overwrites anything.
- **~~No AI.~~ REVISED — this shipped, and it is most of `kg/agent/`.** Three of the four things this line ruled out now exist: `agent/catalog.ts` emits both OpenAI function envelopes and MCP manifests, `agent/loop.ts` is the tool-calling adapter, and `core/provider.ts` names a local-server option explicitly ("vLLM, LM Studio, llama.cpp"). Only "no embeddings" survives — retrieval is lexical, in `agent/retrieve.ts`.

  The three properties this line said would keep the seam free did keep it free, which is the part worth recording: `run` still never imports React, `input` is still introspectable — that is what the catalog is generated FROM — and failures are still structured `ToolError[]`.
